...
def open_file():
    file = askopenfilename(defaultextension=".txt",file=[("All Files","*.*"),("Text Document","*.txt")])
    try:
...
def save_file():
    ...
    if file is None:
        ...
        try:
            ...
            file.close
def cut():
    text_area.event_generator("<<Cut>>")
def copy():
    text_area.event_generator("<<Copy>>")
def paste():
    text_area.event_generator("<<Paste>>")
def about():
